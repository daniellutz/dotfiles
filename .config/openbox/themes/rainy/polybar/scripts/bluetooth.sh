...
CDIR=$(cd "$DIR" && cd .. && pwd)
POWER_ON=$(grep 'BACKGROUND' < "$CDIR"/colors.ini | head -n1 | cut -d '=' -f2 | tr -d ' ')
POWER_OFF=$(grep 'BACKGROUND' < "$CDIR"/colors.ini | head -n1 | cut -d '=' -f2 | tr -d ' ')
...
print_status() {
    if power_on; then
        ...
        if [[ $counter -gt 1 ]]; then
            echo "%{F$POWER_ON}%{T2}%{T-} $icons%{F-}"
        elif [[ $counter -gt 0 ]]; then
            echo "%{F$POWER_ON}%{T2}%{T-} $icons | $device_alias%{F-}"
        fi
    else
